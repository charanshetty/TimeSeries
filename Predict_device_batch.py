...
n_hours = 3 # to t-3
n_features = 15
nscaled = scaler.fit_transform(data)
scaled=nscaled[-13-batch_size:,]
print("range",len(scaled),len(scaled)-n_hours-1-batch_size)
print(data[-10:,0])
actual = []
predicted = []

# load YAML and create model
yaml_file = open('model_device_stateful_1.yaml', 'r')
loaded_model_yaml = yaml_file.read()
yaml_file.close()
loaded_model = model_from_yaml(loaded_model_yaml)
# load weights into new model
loaded_model.load_weights("model_device_stateful_1.h5")
#print("Loaded model from disk")



for i in range(0,len(scaled)-n_hours-batch_size-1,1):
	t = len(scaled)-n_hours-batch_size-1
	print(i,i+n_hours+1+batch_size-1)
	values = scaled[i:i+n_hours+batch_size,]
	print("values",i,len(values))
	print("before",values[n_hours,0],values[n_hours-1,0])
	print(n_hours+i+batch_size)
	values[n_hours+batch_size-1,0]=nscaled[t-(n_hours-2+batch_size),0]
	#print(len(values))
	...
	yhat = loaded_model.predict(test_X,batch_size)
	test_X = test_X.reshape((test_X.shape[0], n_hours*n_features))
	# invert scaling for forecast
	inv_yhat = concatenate((yhat, test_X[:, -14:]), axis=1)
	#print("compare scaled",scaled[i+n_hours-batch_size+1,0],inv_yhat[len(inv_yhat)-1-batch_size+1:,0])
	...
	print("comparing actual ",(13-(i+n_hours+1)),data[-(13-(i+n_hours+1)),0],predicted,data[-(13-(i+n_hours+1))-1440,])
	data[-(13-(i+n_hours+1)),0]=data[-(13-(i+n_hours+1))-1440,0]


print(data[-10:,0],values[-10:,0])
