...
for i in range(2,7,1):
	n_hours = i  # to t-3
	n_features = 15
	reframed = convert_to_time_independent(scaled, n_hours, 5)
	...
	model = Sequential()
	model.add(LSTM(80, batch_input_shape=(1,train_X.shape[1], train_X.shape[2]),return_sequences=True,stateful=False))
	...
	history = model.fit(train_X, train_y, epochs=5, batch_size=1, validation_data=(test_X, test_y), verbose=2, shuffle=False, callbacks=[early_stop])
	...
	with open("model_device_stateful_5.yaml", "w") as yaml_file:
	    yaml_file.write(model_yaml)
	# serialize weights to HDF5
	model.save_weights("model_device_stateful_5.h5")
	print("Saved model to disk")
	 
	# later...
	 
	# load YAML and create model
	yaml_file = open('model_device_stateful_5.yaml', 'r')
	loaded_model_yaml = yaml_file.read()
	yaml_file.close()
	loaded_model = model_from_yaml(loaded_model_yaml)
	# load weights into new model
	loaded_model.load_weights("model_device_stateful_5.h5")
...
combined.to_csv("/users/charan/predicted_device_1722_1440_5.csv", sep=',')
