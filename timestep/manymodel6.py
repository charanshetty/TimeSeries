# uses 5 models t+1, ...t+5 ,..t+1 models for predicting t+6 .
# predict for 6th timestep with accuracy of < 10
import os
...
def getNext5(epochs,index):
    return epochs[index:index+5]
...
subset = []
for epoch, data in timeMap.items():
    next5 = getNext5(epochs,index)
    p=0
    begin = {}
    if (len(next5)==5):
        begin = {needed : timeMap[needed] for needed in getNext(epochs,next5[0])}
        for j in next5:
            resultset = timeMap[j]
        ...
        last = getLast(epochs,next5[len(next5)-1])
        #print(last)
    ...
    if (index == 2000):
        print(predictions)
        predictions.to_csv("/users/charan/predicted_device_1722_6_1.csv", sep=',')
        break
