...
n_hours = 3  # to t-3
n_features = 15
reframed = convert_to_time_independent(scaled, n_hours, 10)
...
model = Sequential()
model.add(LSTM(25, batch_input_shape=(1,train_X.shape[1], train_X.shape[2]),return_sequences=False,stateful=True,activation='tanh'))
#model.add(LSTM(80,return_sequences=True,stateful=True,activation='tanh'))
model.add(Dropout(.2))
#model.add(TimeDistributed(Dense(1,activation='tanh'),input_shape=(29997,3,80 )))	
model.add(Dense(1))
#model.add(Flatten())
...
with open("model_device_stateful_10.yaml", "w") as yaml_file:
    yaml_file.write(model_yaml)
# serialize weights to HDF5
model.save_weights("model_device_stateful_10.h5")
print("Saved model to disk")
 
# later...
 
# load YAML and create model
yaml_file = open('model_device_stateful_10.yaml', 'r')
loaded_model_yaml = yaml_file.read()
yaml_file.close()
loaded_model = model_from_yaml(loaded_model_yaml)
# load weights into new model
loaded_model.load_weights("model_device_stateful_10.h5")
...
combined.to_csv("/users/charan/predicted_device_1722_1440_10.csv", sep=',')
