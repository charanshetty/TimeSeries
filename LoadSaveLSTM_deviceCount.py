#stateful with batch size=1  is better than stateless wrt validation loss , but modelling processing time increases too
#using TimeDistributedDense layer degrades the loss
#having mutliple layers is very specific to timestep
...
model = Sequential()
model.add(LSTM(100, batch_input_shape=(1,train_X.shape[1], train_X.shape[2]),return_sequences=True,stateful=True))
...
with open("model_device_stateful_1_200.yaml", "w") as yaml_file:
    yaml_file.write(model_yaml)
# serialize weights to HDF5
model.save_weights("model_device_stateful_1_200.h5")
...
combined.to_csv("/users/charan/predicted_device_1722_1440.csv", sep=',')
