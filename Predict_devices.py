#predict iteratively ; the predictions worsened with every iteration , as if the error factor is amplified , while the actual behaviour is nullified
...
for i in range(len(scaled)-n_hours-1):
	...
	scaled[n_hours,0]=inv_yhat[len(inv_yhat)-1:,0]
	...
	print("comparing actual ",data[-(12-(i+n_hours)),0],predicted)
	data[-(12-(i+n_hours)),0]=predicted


print(data[-1:,0],values[-1,0])
